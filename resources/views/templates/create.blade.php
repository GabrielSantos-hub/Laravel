        ...
        <div class="card-body p-4">
            <form action="{{ route('templates.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="versao" class="form-control focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-light" value="{{ old('versao', '1') }}" maxlength="20">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="bloco" id="bloco" class="form-select focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-light" required>
                        @foreach (\App\Models\Template::BLOCOS as $codigo => $rotulo)
                            <option value="{{ $codigo }}" @selected(old('bloco', 'A') === $codigo)>{{ $rotulo }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="corpo_template" class="form-control focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-light font-monospace" rows="14" required
                        placeholder="Ex.: com base na arquitetura, use @{{intencao}}, @{{linguagem}}, @{{framework}}, etc.">{{ old('corpo_template') }}</textarea>
                </div>

                <div class="d-flex gap-2">
                    <button type="submit" class="btn focus:ring-2 focus:ring-indigo-500 focus:outline-none text-white px-4" style="background-color: #5b4ce6; border-radius: 6px;">Salvar</button>
                    <a href="{{ route('templates.index') }}" class="btn focus:ring-2 focus:ring-indigo-500 focus:outline-none btn-light border px-4" style="border-radius: 6px;">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
